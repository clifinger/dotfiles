...
if command -v mise &>/dev/null; then
  # Stow mise and local configurations early to ensure config is present
  log "Stowing mise and local configurations..."
  (cd "$HOME/.dotfiles" && stow --target="$HOME" --no-folding mise local)

  # Install dependencies with mise (uses ~/.config/mise/config.toml)
  mise install

  log "✓ mise configured and dependencies installed"
else
...
(cd "$HOME/.dotfiles" && stow --target="$HOME" --no-folding \
  zsh kitty niri waybar gtk-3.0 gtk-4.0 xsettingsd)
